...
<body data-spy="scroll" data-target=".navbar" data-offset="10" id="rests">
    ...
    <header class="header">
        <div class="overlay">
            <h1 class="title"> Notificari administrator</h1>
        </div>  
    </header>
    <!-- End Of Page Header --> 
    ...
    <div class="restaurant-list-view">
    	<% 
    		...
    		ResultSet rs = (ResultSet)request.getSession().getAttribute("notifs");
    		while (rs.next()) {
    			id++;
    			out.println("<div class=\"restaurant-list-element\">");
    			out.println("<div class=\"restaurant-details\">");
    			out.println("<form action = NotifHandlerServlet method = post>");
    			
    			out.println("<h1 class=\"restaurant-name\">Cerere tip Inregistrare " + rs.getString("tip") +" #" + rs.getInt("id") + "</h1>");
    			out.println("<input type = \"hidden\"  name = \"notif" + id + "\" value = \"" + rs.getInt("id") +  "\")>");
    			out.println("<p class=\"restaurant-description\"> " + rs.getString("descriere") + "</p> ");
    			out.println("<input type = \"hidden\"  name = \"actiune"+ id + "\" value = \"" + rs.getString("actiune") +  "\")>");
    			
        		out.println("<input type = \"submit\" class=\"btn btn-primary btn-sm\" name = \"accepta-" + "notif" + id + "\" value = \"Accepta\")>");
        		out.println("<input type = \"submit\" class=\"btn btn-primary btn-sm\" name = \"refuza-" + "notif" + id + "\" value = \"Refuza\")>");
        		out.println("</form>");
    			out.println("</div> </div>");
    		}
    	%>
